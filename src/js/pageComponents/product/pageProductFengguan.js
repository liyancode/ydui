...
//     }]
// }
export default class PageProductFengguan extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: true,
    ...
    handleProductTypeSelectChange(e) {
        if (e === 'all') {
            productService.getAll().then(data => {
                this.setState({products: data["products"], loading: false, one_product_type: e, breadcrumb: '风管产品'+' 共 '+data["products"].length+' 条'});
            });
        } else {
    ...
    handleBackFromAddNewBtnOnclick() {
        this.setState({page: "view_all", breadcrumb: '风管产品'+' 共 '+this.state.products.length+' 条'});
    }
    ...
    render() {
        return (
            <Layout style={{height: '100%'}}>
                <CompnSider defaultMenuKey={['product_fengguan_page']} defaultOpenKeys={['product_m']}/>
                <Layout>
                    <CompnHeader/>
                    <Content>
